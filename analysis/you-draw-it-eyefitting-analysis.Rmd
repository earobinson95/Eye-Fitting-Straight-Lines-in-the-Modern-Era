...
eyefitting_parameter_details <- read_csv("eyefitting_parameter_details.csv")
...
There are a total of `length(unique(eyefitting_model_data$participant_id))` participants and `length(unique(eyefitting_model_data$participant_id))` 'You Draw It' plots completed.
...
eyefitting.lmer.plot <- eyefitting.preds.lmer %>%
  ...
  facet_wrap(~`Parameter Choice`, ncol = 1) +
  theme_bw(base_size = 14) +
  theme(aspect.ratio = 1,
