...
abstract: |
  \svp{How do statistical regression results compare to intuitive, visually fitted results?} Fitting lines by eye through a set of points has been explored since the 20th century. Common methods of fitting trends by eye involve maneuvering a string, black thread, or ruler until the fit is suitable, then drawing the line through the set of points. In 2015, the New York Times introduced an interactive feature, called 'You Draw It'\svp{, where r}eaders are asked to input their own assumptions about various metrics and compare how these assumptions relate to reality. \svp{In this paper, we validate} ‘You Draw It’ as a method for graphical testing, \svp{comparing results to the less technological method utilized in \citet{mosteller1981eye} and extending that study with formal statistical analysis methods.} Results were consistent with those found in the previous study; when shown points following a linear trend, participants tended to fit the slope of the first principal component over the slope of the least-squares regression line. This trend was most prominent when shown data simulated with larger variances. \svp{This study reinforces the differences between intuitive visual model fitting and statistical model fitting, providing information about human perception as it relates to the use of statistical graphics.}
...
Data visualization has become a central tool in modern data science.\svp{XXX this is a bit of a lame intro... we might want to spice this up later, but not critical now}
@unwin2020data defines data visualization as the art of \svp{using} graphic displays to show data. \svp{XXX how does this relate - need to make it flow a bit better. Add additional clauses or connect this to the next thought somehow.}
Graphics illustrate the relationship between two sets of information or variables. 
Graphical displays are useful for data cleaning, exploratory data analysis, identifying trends, evaluating model output, and presenting results.\svp{Right now this reads like bullet points rather than a narrative.}

When creating a graphic, we must consider the design choices most effective for conveying the intended result. \svp{For instance, we must decide to highlight the relationship between two variables in a scatterplot by including a trend line, or adding color to highlight clustering \citep{vanderplas2017clusters}.}
...
<!-- In order to evaluate the effectiveness of a graphic, we must understand cognitive perception and biases related to different types of data structures.  -->
