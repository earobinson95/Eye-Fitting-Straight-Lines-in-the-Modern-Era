...
eyefitting_sim_data <- read_csv("analysis/eyefitting-simulated-data.csv")
...
eyefitting_model_data <- read.csv("analysis/eyefitting-model-data.csv")
...
eyefitting_model_data %>%
  ...
  geom_point(data = eyefitting_simulated_data %>%
               filter(dataset == "point_data", participant_id == "60f510d6896cddc508ea6d3de4b58234", parm_id == "F")  %>% rename(`Parameter Choice` = parm_id),
             aes(x = x, y = y)) +
...
Using the `lmer` function in the lme4 package [@lme4], a linear mixed model (LMM) was fit separately to the OLS residuals and PA residuals, constraining the fit to a linear trend. 
